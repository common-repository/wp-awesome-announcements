(function($) {
    ...
    // Notice Hide
    $('body').on('click', '.jltwann-upgrade-popup .popup-dismiss', function(evt) {
        evt.preventDefault();
        $(this).closest('.jltwann-upgrade-popup').fadeOut(200);
    });

    // Notice Show
    $('body').on('click', '.disabled', function(evt) {
        evt.preventDefault();
        $('.jltwann-upgrade-popup').fadeIn(200);
    });
    ...
    // Search
    $('.jltwann-search-plugins #search-plugins').on('keyup', function() {
        var value = $(this).val();
...
function activatePlugin(element, file) {
    ...
    jQuery.ajax({
        url: JLTWANNCORE.admin_ajax,
        type: "POST",
        data: {
            action: "jltwann_recommended_activate_plugin",
            file: file,
            nonce: JLTWANNCORE.recommended_nonce
        },
...
function installPlugin(element, plugin) {
    ...
    jQuery.ajax({
        ...
        type: "POST",
        data: {
            action: "jltwann_recommended_upgrade_plugin",
            type: 'install',
            plugin: plugin,
            nonce: JLTWANNCORE.recommended_nonce
        },
