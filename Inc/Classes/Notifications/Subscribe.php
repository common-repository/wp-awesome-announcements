...
if ( ! class_exists( 'Subscribe' ) ) {
	...
	class Subscribe extends Notice {
		...
		public function __construct() {
			parent::__construct();
			add_action( 'wp_ajax_jltwann_subscribe', array( $this, 'jltwann_subscribe' ) );
		}
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jltwann_subscribe() {
			check_ajax_referer( 'jltwann_subscribe_nonce' );
		...
		public function notice_content() {
			...
			?>
			<div class="notice notice-jltwann is-dismissible notice-plugin-review notice-<?php echo esc_attr( $this->color ); ?> jltwann-notice-<?php echo esc_attr( $this->get_id() ); ?>">
				<button type="button" class="notice-dismiss jltwann-notice-dismiss"></button>
				<div class="jltwann-subscribe-content">
					<?php $this->set_title(); ?>
					<form style="display:flex">
						<div class="jltwann-plugin-subscribe-input">
						...
						<button type="submit" class="button button-primary jltwann-subscribe-btn"><?php esc_html_e( 'Get Discount', 'wp-awesome-announcements' ); ?></button>
					</form>
				</div>
			</div>
			<?php
		...
		public function plugin_rate_url() {
			return 'https://wordpress.org/plugins/' . JLTWANN_SLUG;
		}
		...
		public function footer_content() {
			...
			<a class="button button-primary rate-plugin-button" href="<?php echo esc_url( $this->plugin_rate_url() ); ?>" rel="nofollow" target="_blank">
				<?php echo esc_html__( 'Rate Now', 'wp-awesome-announcements' ); ?>
			</a>
			<a class="button notice-review-btn review-later jltwann-notice-dismiss" href="#" rel="nofollow">
				<?php echo esc_html__( 'Later', 'wp-awesome-announcements' ); ?>
			</a>
			<a class="button notice-review-btn review-done jltwann-notice-disable" href="#" rel="nofollow">
				<?php echo esc_html__( 'I already did', 'wp-awesome-announcements' ); ?>
			</a>
			<?php
		}
		...
		public function core_script( $trigger_time ) {
			...
			<script>
				jQuery(document).on('submit', '.jltwann-notice-subscribe .jltwann-subscribe-content form', function(e) {
					...
					let formWrapper = form.closest('.jltwann-subscribe-content');
					...
					jQuery.ajax({
							...
							crossDomain: true,
							data: {
								action: 'jltwann_subscribe',
								_wpnonce: '<?php echo esc_js( wp_create_nonce( 'jltwann_subscribe_nonce' ) ); ?>',
								name: name,
								email: email,
							}
						})
						.done(function(response) {
							formWrapper.hide().after('<p class="jltwann--notice-message"><strong>' + response.data + '</strong><p>');
							let subsTimeout = setTimeout(function() {
								jltwann_notice_action(null, form, 'disable');
